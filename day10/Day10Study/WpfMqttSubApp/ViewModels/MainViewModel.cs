...
namespace WpfMqttSubApp.ViewModels
{
    public partial class MainViewModel : ObservableObject, IDisposable
    {
        ...
        public MainViewModel(IDialogCoordinator coordinator)
        {
            ...
            DatabaseHost = "210.119.12.56"; // 본인 pc 아이피 MySQL 서버 그대로
        ...
        private async Task SaveSensingData(SensingInfo data)
        {
            ...
            try
            {
                if (connection.State == System.Data.ConnectionState.Open) 
                {
                    ...
                    cmd.Parameters.AddWithValue("@fan", data.F == "ON" ? 1 : 0);
                    cmd.Parameters.AddWithValue("@vul", data.V == "ON" ? 1 : 0);
                    cmd.Parameters.AddWithValue("@real_light", data.RL == "ON" ? 1 : 0);
                    cmd.Parameters.AddWithValue("@chaim_bell", data.CB == "ON" ? 1 : 0);

                    int result = await cmd.ExecuteNonQueryAsync();   // 이전까지는 cmd.ExecuteNonQuery()
                }
            }
            catch (Exception ex)
            {
