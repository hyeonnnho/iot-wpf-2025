...
namespace MovieFinder2025.ViewModels
{
    public partial class MoviesViewModel : ObservableObject
    {
        ...
        public MoviesViewModel(IDialogCoordinator coordinator)
        {
            ...
            _timer.Interval = TimeSpan.FromSeconds(1);  // 1초 마다 변경
        ...
        {
            string tmdb_apikey = "c10db2b62f99fabb7d6fcb92cf2e2433"; // TMDB 에서 신청한 API키
            string encoding_movieName = HttpUtility.UrlEncode(movieName, Encoding.UTF8);    // 입력한 한글을 유니코드로 변경
            ...
            // OpenAPI 실행할 웹 객체, WebRequest, WebResponse
            // HttpClient로 대체할 것
            //WebRequest req = null;
        ...
        {
            //await this.dialogCoordinator.ShowMessageAsync(this, "즐겨찾기 추가", SelectedMovieItem.Count);
        ...
        public async Task ViewFavoriteMovies()
        {
            ...
            try
            {
                using (MySqlConnection conn = new MySqlConnection(Common.CONNSTR))
                {
                    ...
                    while (reader.Read())
                    {
                        movieItems.Add(new MovieItem
                        {
                            ...
                            Backdrop_path = reader.IsDBNull(reader.GetOrdinal("backdrop_path"))
                                        ? string.Empty  // 기본값
                                        : reader.GetString("backdrop_path"),
                            ...
                            Poster_path = reader.IsDBNull(reader.GetOrdinal("poster_path"))
                                        ? string.Empty  // 기본값
                                        : reader.GetString("poster_path"),
                ...
                Common.LOGGER.Info(SearchResult + " 검색 완료!");
            }
        ...
        public async Task ViewMovieTrailer()
        {
            ...
            view.Owner = Application.Current.MainWindow;    // 부모 창의 중앙에 위치
